...
fun getAllStates(): List<State> {
    try {
        val states = org.json.JSONObject(stateData).keys()
...
fun getLocalGovernmentsForState(state: State): List<Lga> {
    return try{
        val list = mutableListOf<Lga>()
        val currentState = org.json.JSONObject(stateData).getJSONArray(state.name)
        for (i in 0 until currentState.length()){
            val lga = Lga(stateId = state.id, name = currentState[i] as String)
            list.add(lga)
        }
        list
    }catch (e: Exception){
